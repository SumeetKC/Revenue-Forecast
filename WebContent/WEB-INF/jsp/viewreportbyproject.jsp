...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#projects").change(function() {
			...
			if ($("#projects").val() == "") {
				alert("Not Valid Project");
				return false;
			} else {
				$("#projectreportform").submit();
			}
...
<body>
	<div align="center">
		<form method="post" action="showreportbyproject.html"
			id="projectreportform">
			<!-- target="resframe"  -->
			...
			<select name="projects" id="projects">
				...
				<c:forEach var="projects" items="${projects}">
					<option value="${projects.key}">${projects.key}-
						${projects.value}</option>
				</c:forEach>
				...
				<table border="2"
					...
					<c:forEach items="${basedata}" var="baselist">
						<tr>
							...
							<td>${baselist.name}</td>
							<td style="background-color: #00FF00">${baselist.project}</td>
							<td style="background-color: #00FF00">${baselist.description}</td>
							<td>${baselist.pm_id}</td>
							<td>${baselist.pm_name}</td>
